...
import { deleteAccount, deletePosition, confirmRecurringIncome, deleteRecurringIncome } from '@/lib/actions'
import type { AccountWithLatestSnapshot, Position, PositionWithQuote, RecurringIncome } from '@/types'
...
function DeltaBadge({ delta }: { delta: number | null | undefined }) {
  ...
  const colorClass = delta === null
    ...
    : delta > 0 ? 'text-[var(--primary)]' : delta < 0 ? 'text-[#ef4444]' : 'text-muted-foreground'
  return (
    <p className={`font-mono text-[11px] tabular-nums mt-0.5 ${colorClass}`}>
...
export function AccountsList({
  ...
  const { showAccounts, setShowAccounts, showPositions, setShowPositions } = useVisibility()
  ...
  const isEmpty = accounts.length === 0 && positionsWithQuotes.length === 0 && manualPositions.length === 0 && incomes.length === 0
      ...
      {incomes.length > 0 && (
        <div>
          <GroupHeader
            ...
            total={formatCurrency(incomesTotal)}
            totalColor="text-[var(--primary)]"
            open={openGroups.entrate}
            onToggle={() => toggleGroup('entrate')}
          />
          {openGroups.entrate && incomes.map((income) => {
